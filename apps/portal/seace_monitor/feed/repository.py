...
sobre la tabla `processes`/`FeedItem`. Centralizar el acceso aquí permite:
...
from ..db.models import FeedItem, ProcessStatus, TenantFeedDecision
...
class FeedRepository:
    ...
    # 0.3e no consulten `FeedItem.promoted_at` directamente.

    @staticmethod
    def feed_pure_predicate():
        """SQL: items sin promoción (feed puro, purgable)."""
        return FeedItem.promoted_at.is_(None)

    @staticmethod
    def promoted_predicate():
        """SQL: items promovidos (pipeline-bound)."""
        return FeedItem.promoted_at.isnot(None)

    def is_promoted(self, process: FeedItem) -> bool:
        return is_promoted(process)

    def is_feed_pure(self, process: FeedItem) -> bool:
    ...
    def promoted_ids(self) -> set[int]:
        ...
        return {
            row[0]
            for row in self.session.query(FeedItem.id)
    ...
    def find_by_ref(
        ...
        """
        return (
            self.session.query(FeedItem)
            .filter(
                FeedItem.source == source,
                FeedItem.entity_id == entity_id,
                FeedItem.source_ref == source_ref,
            )
            .one_or_none()
        )
    ...
    def query_by_status(
        ...
        *,
        ...
        """
        query = self.session.query(FeedItem).filter(
            FeedItem.status.in_(list(statuses))
        )
    ...
    def by_status_for_entity(
        ...
        *,
        ...
        return (
            ...
            .filter(FeedItem.entity_id == entity_id)
            .all()
        )
    ...
    # `TenantFeedDecision` en vez de los campos `FeedItem.status`/`auto_reject_exempt`.
    ...
    # Resuelven la decisión efectiva combinando overlay + campos legacy de `FeedItem`,
    ...
    def effective_autorejected_ids(
        ...
        rows = (
            self.session.query(FeedItem.id)
            .outerjoin(
                decision,
                and_(
                    decision.feed_item_id == FeedItem.id,
            ...
            .filter(
                or_(
                    ...
                    and_(
                        FeedItem.status == ProcessStatus.autorejected,
    ...
    def decision_for(
        self, process: FeedItem, tenant_id: str = DEFAULT_TENANT_ID
    ) -> str | None:
