...
- If plan/diagram candidates exist and no validated visual analysis JSON exists,
  it writes step_1_planos_candidates_pending.json and exits with code 23.
...
from common import sanitize_filename  # noqa: E402
...
STEP_FILE_NAMES = [
    ...
    "step_1_3_blocked_modal_billing_limit.json",
]
...
def process_xlsx(xlsx: list[Path], d: dict[str, Path], overwrite: bool) -> None:
    for xp in xlsx:
        t0 = time.time()
        manifest_path = d["xlsx_split"] / f"{xp.stem}_split_manifest.json"
        if overwrite or not valid_file(manifest_path):
...
def process_pdf_inputs(docx: list[Path], pdfs: list[Path], d: dict[str, Path], overwrite: bool) -> None:
    for dp in docx:
        t0 = time.time()
        out = d["pdfs"] / (dp.stem + ".pdf")
        if overwrite or not valid_file(out, 100):
            run(["soffice", "--headless", "--convert-to", "pdf", "--outdir", d["pdfs"], dp], timeout=300)
        if not valid_file(out, 100):
...
def clean_pdfs(d: dict[str, Path], overwrite: bool) -> list[Path]:
    ...
    for pdf in sorted(d["pdfs"].glob("*.pdf")):
        ...
        if overwrite or not (valid_file(clean_pdf, 100) and valid_file(report, 10) and valid_file(page_analysis, 10)):
            run([
                PY,
        ...
        if not valid_file(clean_pdf, 100):
            raise RuntimeError(f"Cleaning no produjo PDF válido: {clean_pdf}")
        if not valid_file(page_analysis, 10):
...
def audit_and_build_plan_pages(clean_pdfs: list[Path], d: dict[str, Path], overwrite: bool) -> None:
    ...
    for clean_pdf in clean_pdfs:
        ...
        if overwrite or not valid_file(audit_path, 10):
            run([
                PY,
    ...
    if pending:
        ...
        append_log(d["logs"], f"## [BLOQUEADO] Paso 1.2b — análisis visual requerido\n- Documentos con candidatos pendientes: {len(pending)}\n- Marker: `{marker}`\n- Acción requerida: lanzar subagente visual OpenClaw/Gemini y escribir `planos_analysis_{{stem}}.json` validado.")
        print(f"VISUAL_CANDIDATES_PENDING {marker}")
        raise SystemExit(23)


def modal_docling(clean_pdfs: list[Path], d: dict[str, Path], overwrite: bool) -> None:
    md_outputs: list[str] = []
    for clean_pdf in clean_pdfs:
        t0 = time.time()
        stem = clean_pdf.stem.removesuffix("_clean")
        preocr = d["preocr"] / f"{stem}_preocr.pdf"
        src = preocr if valid_file(preocr, 100) else clean_pdf
        expected = d["normal"] / (sanitize_filename(src) + "_modal_docling.md")
        if overwrite or not valid_file(expected, 100):
            run([PY, "scripts/extractors/modal_docling_extract.py", src, "--output-dir", d["normal"], "--max-wait", "1800"], timeout=2100)
        if not valid_file(expected, 100):
            raise RuntimeError(f"Modal Docling produjo output ausente/pequeño: {expected}")
        md_outputs.append(str(expected))
        append_log(d["logs"], f"## Paso 1.3 — Modal Docling PDF→Markdown\n- Input: `{src}`\n- Output: `{expected}`\n- Duración: {time.time()-t0:.1f}s")
    (d["art"] / "step_1_3_outputs.json").write_text(json.dumps({"markdown_outputs": md_outputs}, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    process_xlsx(xlsx, d, args.overwrite)
    process_pdf_inputs(docx, pdfs, d, args.overwrite)
    clean = clean_pdfs(d, args.overwrite)
    audit_and_build_plan_pages(clean, d, args.overwrite)
    modal_docling(clean, d, args.overwrite)
