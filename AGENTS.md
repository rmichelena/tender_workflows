...
- SEACE watchlist (`descargada`/`analizada`): refresh cronograma+files every 3h; unread flag until detail viewed; bold in lists, sidebar badge; detail diff (prior cronograma values, new files bold); re-analyze archives prior analysis with timestamp.
...
- Entidades SEACE: catálogo OSCE descargado a tabla `Entity`; entidades activas para scan vía flag `activa` en Settings → entidades (UI); dashboard cuenta solo seleccionadas; página entidades muestra `nn seleccionadas de xx entidades`; legacy `entities.csv` removed.
