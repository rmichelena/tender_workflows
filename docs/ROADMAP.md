...
| W1 | ✅ **Watchlist TTL adaptativo:** intervalo base (`watchlist_refresh_interval`, default 3h) y urgente (`watchlist_refresh_interval_urgent`, default 45m) cuando fin consultas/presentación cae dentro de `watchlist_urgent_horizon` (default 48h). Lógica en `watchlist_refresh.py`; aplica a SEACE y ADP. Configurable en `config.yaml`. |
| W2 | ✅ **Timezone en el historial de cambios (watchlist):** entradas solo con documentos nuevos muestran la **fecha de publicación del portal**; cronograma/otros usan la hora del scan en `display_timezone` (default America/Lima, configurable en `config.yaml`). Pendiente: UI de settings para TZ. |
| W5 | **Test dedupe autorejected+promoted simultáneo:** `_claimed_nomenclatura_map` maneja el caso (promovidos en loop 1, autorechazados feed-puro en loop 2) pero no hay test dedicado; añadir si se toca de nuevo el scanner de re-publicación. |
| W4 | **Overlay autoreject sin `config.tenant_id` explícito:** writes/reads de `TenantFeedDecision` usan `DEFAULT_TENANT_ID` por defecto, no `config.tenant_id`. Irrelevante hoy (single-tenant), pero con multi-tenant contra DB compartida las decisiones de un tenant podrían filtrarse a otro; la columna `tenant_id` ya existe en el schema. Abordar al cablear multi-tenant lógico (Fase 0.6 / Fase 4). |
